...
final class FeedLocalizationTests: XCTestCase {
    func test_localizedStrings_haveKeysAndValuesForAllSupportedLocalizations() {
        ...
        let localizationBundles = allLocalizationsBundles(in: presentationBundle)
    ...
    private func allLocalizationsBundles(in bundle: Bundle, file: StaticString = #file, line: UInt = #line) -> [LocalizedBundle] {
    ...
    private func allLocalizedStringKeys(in bundles: [LocalizedBundle], table: String, file: StaticString = #file, line: UInt = #line) -> Set<String> {
        return bundles.reduce([]) { acc, current in
            guard let path = current.bundle.path(forResource: table, ofType: "strings"),
                ...
                XCTFail("coult not load localized strings for localization: \(current.localization)", file: file, line: line)
                return acc
            }
